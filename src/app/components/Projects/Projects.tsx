    ...
    <article className="flex flex-col p-5 rounded-xl bg-white/40 dark:bg-white/5 backdrop-blur-sm border border-white/60 dark:border-white/10 shadow-sm hover:shadow-md hover:bg-white/60 transition-all duration-200">
      ...
      <div className="flex items-start justify-between gap-4 mb-3">
        <div className="flex items-center gap-2.5">
          ...
          <h3 className="font-bold text-(--gaudi-ink) text-base sm:text-lg tracking-tight font-mono">{project.name}</h3>
        </div>
        <div className="flex items-center gap-3 shrink-0">
          <span className={`type-label ${status.label}`}>{project.status}</span>
          <span className="type-label text-ink/30">{project.year}</span>
          {project.location && (
            <span className="hidden sm:inline type-label text-ink/20">{project.location}</span>
          )}
        </div>
      </div>
      ...
      {/* Description */}
      <p className="type-body-sm text-ink/70 leading-relaxed mb-4">{project.description}</p>
  ...
  return (
    <section>
      <div className="mb-6 mt-8">
        <h3 className="text-xl sm:text-2xl font-bold text-(--gaudi-ink) tracking-[-0.02em] mb-1">Projects &amp; Libraries</h3>
      </div>
